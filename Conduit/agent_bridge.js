...
function parseArgs(argv) {
    const args = { shadow: false, archive: false, bootstrap: false, clear: false, timeline: false, clearHistory: false, retry: false, unignore: false, sweep: false, cleanBackups: false, approve: false, mode: null };
...
async function updateState() {
    ...
    const logToHistory = (msg) => {
        let historyObj = { events: [], maxEntries: 500 };
        if (fs.existsSync(HISTORY_PATH)) {
            try {
                historyObj = JSON.parse(fs.readFileSync(HISTORY_PATH, 'utf8'));
                if (!historyObj.events) historyObj.events = [];
            } catch (e) { historyObj = { events: [] }; }
        }
        historyObj.events.push({
            timestamp: new Date().toISOString(),
            agent: args.agent || 'system',
            event: msg
        });
        if (historyObj.events.length > 500) historyObj.events = historyObj.events.slice(-500);
        atomicWrite(HISTORY_PATH, historyObj);
    };
    ...
    try {
        ...
        if (args.insight) {
            const LEARNINGS_PATH = path.join(CONDUIT_DIR, 'learnings.json');
            let learnings = { sessions: [], cumulativeInsights: [], lastSession: new Date().toISOString() };
            if (fs.existsSync(LEARNINGS_PATH)) {
                try { learnings = JSON.parse(fs.readFileSync(LEARNINGS_PATH, 'utf8')); } catch (e) { }
            }
            learnings.cumulativeInsights.push({
                insight: args.insight,
                timestamp: new Date().toISOString(),
                agent: args.agent || 'bridge'
            });
            if (learnings.cumulativeInsights.length > 100) learnings.cumulativeInsights = learnings.cumulativeInsights.slice(-100);
            learnings.lastSession = new Date().toISOString();
            atomicWrite(LEARNINGS_PATH, learnings);
            console.log(`💡 Conduit: Captured cumulative insight - "${args.insight}"`);
            return;
        }

        if (args.mode) {
            const validModes = ['research', 'creative', 'prototype', 'standard'];
            const selectedMode = args.mode.toLowerCase();

            if (!validModes.includes(selectedMode)) {
                console.error(`❌ Invalid mode: ${args.mode}. Valid modes: ${validModes.join(', ')}`);
                return;
            }
            ...
            const modeDescriptions = {
                research: '🔬 Research & Planning - Deep analysis and documentation focus',
                creative: '🎨 Creative Mode - UI/UX experimentation and feature innovation',
                prototype: '⚡ Prototype Mode - Rapid MVP development',
                standard: '🎯 Standard Mode - Balanced autonomous operation'
            };

            console.log(`🎛️ Agent Mode Set: ${modeDescriptions[selectedMode]}`);
            logToHistory(`MODE CHANGE: Set to ${selectedMode.toUpperCase()}`);
            return;
        }
